...
\begin{solucio}
(a) \'{E}s un predicat d'una variable $x$ que \'{e}s vertader quan, per
...
Podem expressar-lo com $x+y<1$. \'{E}s clar que quan donem valors a $x$ i $y$
s'obt\'{e} una proposici\'{o}.
\end{solucio}
...
els valors seg\"{u}ents (1) $x=1,y=3$; (2) $x=2,y=1$.
\end{exercici}

\begin{solucio}
Per a l'expressi\'{o} $\lnot P(x,y)\vee Q(x,y)$, tenim:
(1) amb $x=1$ i $y=3$, \'{e}s vertadera, ja que $Q(1,3)$ \'{e}s vertadera;
(2) amb $x=2$ i $y=1$, tamb\'{e} \'{e}s vertadera, ja que $P(2,1)$ \'{e}s falsa.

Per a l'expressi\'{o} $\lnot(P(x,y)\longleftrightarrow Q(x,y))$, tenim:
(1) amb $x=1$ i $y=3$, \'{e}s falsa, ja que $P(1,3)$ i $Q(1,3)$ s\'{o}n
ambdues vertaderes; (2) amb $x=2$ i $y=1$, tamb\'{e} \'{e}s falsa, ja que
...
$. Com a conseq\"{u}\`{e}ncia, obtenim que $X\longrightarrow\left(  X\wedge Y\right)
...
conseq\"{u}\`{e}ncia no es construir\`{a} un estadi nou. Aix\`{o} \'{e}s fals com
...
y\right)  \right)  $, si $N(x)=$\textquotedblleft$x$ \'{e}s un nombre
...
Sumant tenim:%
\begin{equation}
M+N+P=\frac{a}{b+c}+\frac{b}{a+c}+\frac{c}{a+b}+3 \label{eq:nesbitt-suma}%
\end{equation}
Ara b\'{e},%
\[
...
\]
D'aqu\'{\i} i (\ref{eq:nesbitt-suma}), surt%
\[
...
\mathbb{Z}$. Aleshores $mn$ \'{e}s parell com volíem demostrar.
\end{solucio}

\begin{exercici}
Proveu per reducci\'{o} a l'absurd que els \'{u}nics enters consecutius no
...
\begin{solucio}
(1) Constru\"{\i}m una prova d'inducci\'{o} sobre $n$, essent $P(n)=$
\textquotedblleft$%
%TCIMACRO{\dsum \limits_{k=0}^{n}}%
%BeginExpansion
{\displaystyle\sum\limits_{k=0}^{n}}
...
(2) Constru\"{\i}m una prova d'inducci\'{o} sobre $n$, essent $P(n)=$
...
enters $q,r$ i s\'{o}n \'{u}nics tals que $a=bq+r$, amb $0\leq r<b$.
\end{exercici}
...
$q^{\prime}$ que compleixen $a=bq^{\prime}+r^{\prime}$, amb $0\leq r^{\prime
}<b$. Aleshores, es t\'{e}%
\begin{align*}

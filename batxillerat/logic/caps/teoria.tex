...
compleix $(x-y)(x+y)=x^{2}-y^{2}$' \'{e}s una proposici\'{o} vertadera. Tamb%
\'{e} hi ha proposicions falses amb variables com, per exemple, `Per a tot
nombre real $x$ es t\'{e} $\sqrt{x^{2}}=x$'.

Tamb\'{e} hi ha enunciats que encara no sabem si s\'{o}n certs o no, que s%
...
i $n>2$ es t\'{e} $a^{n}+b^{n}\neq c^{n}$, per\`{o} en aquest cas, el 1994,
...
\item Els nombres enters parells m\'{e}s grans que un nombre irracional donat.
...
paral\textperiodcentered lela a $x+y=1$' que \'{e}s vertadera. De fet el
...
P(98)$ si i nom\'{e}s si $Q(2)$ i $R(3)$, on $P$ \'{e}s el predicat `ser m\'{u}%
...
\`{a} ben formada. La ra\'{o}, per exemple, est\`{a} en qu\`{e} $\wedge$
...
\begin{exemple}
Si $p,q$ i $r$ s\'{o}n variables proposicionals, llavors
\[
\left( \left(\lnot p\vee q\right) \wedge\left( \lnot r\longrightarrow q\right) \right)
\longrightarrow\left( p\longrightarrow r\right)
\]
\'{e}s una forma proposicional tautol\`{o}gica?
\end{exemple}
...
\section{Implicaci\'{o} l\`{o}gica\label{sec:implicacio-logica}}
...
\section{Equival\`{e}ncia l\`{o}gica\label{sec:equivalencia-logica}}
...
\`{e}ncia} \label{sec:lleis-inferencia} de la l\`{o}gica proposicional, es detallen a continuaci%
\'{o}.
...
implicacions l\`{o}giques de la secci\'{o} \ref{sec:implicacio-logica}, tamb\'{e} les equival%
\`{e}ncies l\`{o}giques de la secci\'{o} \ref{sec:equivalencia-logica} com, per exemple, les lleis
de De Morgan.
...
Com a conseq\"{u}\`{e}ncia, podem dedu\"{\i}r $s\wedge\lnot s$, que \'{e}s una
...
no buit; i la proposici\'{o} $\forall x\in X,P(x)$, com%
\begin{equation*}
...
A difer\`{e}ncia de les equival\`{e}ncies discutides a la secci\'{o} \ref{sec:equivalencia-logica}%
...
secci\'{o} \ref{sec:lleis-inferencia}, s'utilitzen amb el mateix prop\`{o}sit, que \'{e}s
...
\begin{solucio}
No \'{e}s correcte, perqu\`{e} en el pas 5 el s\'{\i}mbol `$a$' ha aparegut
abans.
\end{solucio}
...
que simbolitzem per $x\mid y$. La seva definici\'{o} \'{e}s: $x\mid y$ si i nom\'{e}s si existeix $k\in\mathbb{Z}$ tal que $kx=y$%
. Ara hem d'escollir una estrat\`{e}gia per a la prova. En aquest cas escollim la
prova directa: suposem que $a\mid b$ i $b\mid c$ (hip\`{o}tesis) i hem de
...
que $(\sqrt{x}-\sqrt{y})^{2}\geq0$. Desenvolupant aquesta \'{u}ltima expressi%
...
x+y & \geq2\sqrt{xy}
\end{align*}
Finalment, dividint tots dos costats per $2$, es t\'{e} el resultat que
volíem:%
\begin{equation*}
...
\begin{exemple}
\label{ex:n2-parell}Provar que per a qualsevol nombre enter $n$, si $n^{2}$ \'{e}s
...
\mathbb{Z}$, s'obt\'{e} que $n^{2}$ \'{e}s senar, com volíem demostrar.
\end{solucio}
...
recordem la definici\'{o} d'un nombre racional diferent de zero: $a\neq0$
\'{e}s racional si i nom\'{e}s si existeixen enters no nuls $m,n$ tals que $a=\dfrac{m}{n}$%
...
Per demostrar aquesta proposici\'{o} universal, considerem dos nombres enters
no nuls $m,n$. Hem de demostrar que no \'{e}s possible que $\dfrac{m}{n}=\sqrt {%
2}$. L'estrat\`{e}gia ser\`{a} fer la prova per reducci\'{o} a l'absurd:
suposem que $m$ i $n$ s\'{o}n enters no nuls i que $\dfrac{m}{n}=\sqrt{2}$ (hip%
\`{o}tesi), hem de trobar una contradicci\'{o} (tesi). No \'{e}s restrictiu
...
com hem provat en l'exemple \ref{ex:n2-parell}. Llavors, $m=2k$, on $k\in\mathbb{Z}$.
...
m}{n}=\sqrt{2}$, i, com que $m$ i $n$ s\'{o}n arbitraris, $\sqrt{2}$ no \'{e}s
racional.
...
\begin{solucio}
Primer, escrivim simb\`{o}licament l'enunciat: $\lnot\left( \exists x\in%
...
\begin{exemple}
Volem provar que si $x$ i $y$ s\'{o}n dos nombres reals, llavors $xy=0$ si i nom\'{e}s si $x=0$
o $y=0$.
\end{exemple}
...
\[
A=\left\{ a\in\mathbb{N}: \exists b\in\mathbb{N}\text{ i }a>b>0\text{ i }\Phi=\dfrac{a}{b}\right\}.
\]
Si $A=\emptyset$, llavors $\Phi$ \'{e}s
...
compleixen la propietat. Llavors, de \eqref{eq:phi-quadratica} es t\'{e}%
\begin{align*}
...
\subsection{Demostracions per inducci\'{o}\label{subsec:induccio}}
...
Un altre exemple són les definicions per \textbf{recursi\'{o}}. La successi%
...
obtenim que l'\`{a}rea del quadrat $BIHC$ de catet oposat a l'angle recte,
...
\'{e}s el conjunt $\mathbb{N}$, l'exist\`{e}ncia del qual ve donada pels
...
que hem tractat en l'apartat \ref{subsec:induccio}.
...
\'{\i}stica del conjunt dels nombres naturals: tot subconjunt $K$ no buit de
...
principi d'inducci\'{o}: Considerem un conjunt no buit de nombres naturals $K$
sense cap primer element. Provarem per inducci\'{o} que cap nombre natural no
pertany a $K$. El cas base \'{e}s $1\notin K$: si $1\in K$, com que $1$ \'{e}s
el primer element de $\mathbb{N}$, tamb\'{e} seria el primer element de $K$,
contra la hip\`{o}tesi. Suposem ara que $1,2,\ldots,n$ no pertanyen a $K$ i
provem que $n+1\notin K$. Si $n+1\in K$, aleshores cap nombre natural m\'{e}s
petit que $n+1$ pertany a $K$, i per tant $n+1$ seria el primer element de
$K$, contradient novament la hip\`{o}tesi. Aix\'{\i}, per inducci\'{o}, cap
nombre natural no pertany a $K$, \'{e}s a dir, $K=\emptyset$, en contradicci\'{o}
amb el fet que $K$ era no buit. Per tant, $K$ t\'{e} primer element.
\end{prova}
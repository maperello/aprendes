...
\subsection{Operacions combinades amb nombres enters}

En les operacions combinades amb nombres enters cal respectar l'ordre habitual de les operacions:
\begin{enumerate}
	\item parèntesis;
	\item potències i arrels;
	\item multiplicacions i divisions;
	\item sumes i restes.
\end{enumerate}
També cal tenir una cura especial amb els signes. Recordem que el producte o el quocient de dos nombres enters amb el mateix signe és positiu, mentre que el producte o el quocient de dos nombres enters amb signes diferents és negatiu.

\begin{observacio}
La divisió de dos nombres enters no sempre dona un nombre enter. En aquesta subsecció només farem divisions exactes, és a dir, divisions el resultat de les quals és un nombre enter.
\end{observacio}
...
\begin{solucio}
Agrupem els termes positius i els termes negatius:
\[
	-7+12-5-9
	=
	12-(7+5+9).
...
\[
	...
	18:(-3)-2\cdot(-5).
\]
Ara fem la divisió i el producte:
\[
	18:(-3)=-6
\]
i
...
\begin{solucio}
Aquest exercici és important perquè mostra el paper dels parèntesis.
...
\[
	\sqrt{49}=7,
\]
perquè l'arrel quadrada principal és no negativa.
...
Calculeu:
\[
	\left[(-4)^2-3\cdot(6-10)\right]:(-7).
\]
\end{exercici}

\begin{solucio}
Primer resolem el parèntesi interior:
\[
	6-10=-4.
\]
Aleshores
\[
	3\cdot(6-10)=3\cdot(-4)=-12.
\]
També tenim
\[
	(-4)^2=16.
...
Així,
\[
	\left[(-4)^2-3\cdot(6-10)\right]:(-7)=-4.
\]
\end{solucio}
...
Calculeu:
\[
	-5+2\left[3^2-\sqrt{16}-(-2)^3\right].
\]
\end{exercici}

\begin{solucio}
Primer calculem les potències i l'arrel:
\[
	3^2=9,
	\qquad
	\sqrt{16}=4,
...
Per tant,
\[
	-5+2\left[3^2-\sqrt{16}-(-2)^3\right]=21.
\]
\end{solucio}

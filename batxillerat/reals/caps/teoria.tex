...
h^{2}=c_{1}^{2}+c_{2}^{2}
\end{equation*}%
on $h$ indical costat de major longitud i $c_{1},c_{2}$ els
...
un d’ells és la desena part dl’anterior, per la propietat dels
...
figura 11 es mostra amb més de detall al tractar un exemple. 
\begin{center}
...
1.237000...$. Sabem que cada una d’aquestes col·leccions de intervals
...
les dues successions d’intervals encaixats definen el mateix nombre $%
...
En resum, tot punt de la rectaestà representat per un nombre
amb (possiblemente) infinites xifres decimals, però identificant els
decimals exactes amb els decimals de período 9 i amb els de período 0.

En aquesta recta, entre dos racionals existeix sempre un altre racional, entre dos
irracionals existeix sempre un racional tan próximo a él com es
vulgui (Aquesta idea procede de la forma com s’han definit els nombres
...
reals deu ferse amb aproximacions; per sumar i multiplicar dos nombres reals caldrà sumar i multiplicar les seves aproximacions. També
 poden ferse les operacions geomètricament, utilitzant les
operacions amb segments. Admitiremos que les operacions anteriors en $%
\mathbb{R}$ compleixen les mateixes propietats que en $\mathbb{Q}$. D’aquesta
manera, el conjunt dels nombres reals té estructura de cos. Com
...
En la pràctica, al treballar amb els nombres reals que són
...
És clar que treballar amb una aproximació, en lugar de treballar amb el
...
correctes; a l’apartat següent lo veurem amb alguns exemples.
...
\sqrt{2}$ i $\pi$ amb aproximació fins a les deu mil·lèsimes, es
dir, $\sqrt{2}=1.4142...$ i $\pi =3.1415...$ i, d’aquesta manera, obtenim 
\begin{equation*}
...
suficients per provar aquests resultats) que les operacions de suma y
producte de nombres reals compleixen les mateixes propietats que la suma y
el producte de nombres racionals.

La operació suma de nombres reals compleix les següents
propietats:

\begin{enumerate}
\item Ley de composición interna: $a+b\in \mathbb{R}$, per tot $a,b\in 
\mathbb{R}$

\item Asociativa: $a+(b+c)=(a+b)+c$, per tot $a,b,c\in \mathbb{R}$

\item Elemento neutro: existeix $0\in \mathbb{R}$ tal que $a+0=0+a=a$, per
tot $a\in \mathbb{R}$.

\item Conmutativa: $a+b=b+a$, per tot $a,b\in \mathbb{R}$

\item Elemento simétrico: per cada$a\in \mathbb{R}$ existeix $-a\in 
\mathbb{R}$, anomenado \textbf{oposat} de $a$, tal que $a+(-a)=(-a)+a=0$
\end{enumerate}

La operació producte de nombres reals compleix les següents
propietats:

\begin{enumerate}
\item[6.] Ley de composición interna: $a\cdot b\in \mathbb{R}$, per
tot $a,b\in \mathbb{R}$

\item[7.] Asociativa: $a\cdot (b\cdot c)=(a\cdot b)\cdot c$, per tot $%
a,b,c\in \mathbb{R}$

\item[8.] Elemento neutro: existeix $1\in \mathbb{R}$ tal que $a\cdot 1=1\cdot
a=a$, per tot $a\in \mathbb{R}$

\item[9.] Conmutativa: $a\cdot b=b\cdot a$, per tot $a,b\in \mathbb{R}$

\item[10.] Elemento simétrico: per cada$a\in R$ amb $a\neq 0$ existeix $%
1/a\in \mathbb{R}$, anomenado \textbf{invers} de $a$, tal que $a\cdot
(1/a)=(1/a)\cdot a=1$
\end{enumerate}

Finalment, les dos operacions compleixen la propietat

\begin{enumerate}
\item[11.] Distributiva del producte respecto de la suma:%
\begin{equation*}
\begin{array}{c}
...
senzilla. A partir dl’ordre en $\mathbb{R}$ definirem el valor absolut de
un nombre real i, a partir del valor absolut definirem la distància
entre dos nombres reals. Finalment, distinguiremos alguns
...
\mathbb{R}$. Això vol dir, per exemple, que al considerar $\mathbb{Q}$
...
una idea clara dl’ordre en $\mathbb{R}$. No obstant això, hi ha una forma de
...
resultats en notació científica. Per exemple, al calcular la
potència $100^{10}$%
\begin{equation*}
\begin{array}{ccccccccccc}
...
aproximacions successives de nombres racionals o bé per l’algorisme clàssic d’extracció d’arrels quadrades). No obstant això, podem
...
no és possible ja que $-2\neq 2$. Aquesta contradicció es deu al fet que al efectuar potències d’exponent par, per les regles dels signes,

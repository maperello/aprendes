...
\begin{solucio}
Sigui $\cat{FinSet}$ la categoria dels conjunts finits i aplicacions, i $\cat{S}$ la subcategoria plena amb objectes els $\underline{n}$. El functor inclusió $\iota\colon\cat{S}\hookrightarrow\cat{FinSet}$ és:
\begin{itemize}
\item \textbf{ple i fidel}, per ser $\cat{S}$ una subcategoria plena: entre dos objectes $\underline{m},\underline{n}$, els morfismes a $\cat{S}$ són \emph{tots} els de $\cat{FinSet}$, és a dir, totes les aplicacions $\underline{m}\to\underline{n}$;
\item \textbf{essencialment exhaustiu}: tot conjunt finit $X$ té alguna cardinalitat $n$, i llavors és isomorf (mitjançant una bijecció) a $\underline{n}$, que és un objecte de $\cat{S}$.
\end{itemize}

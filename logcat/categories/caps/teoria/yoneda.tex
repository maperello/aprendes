...
\begin{observacio}
Aquesta demostració és el patró que retrobarem sempre: quan un objecte es defineix per una propietat universal ---\enquote{hi ha un únic morfisme tal que…}---, dos objectes qualssevol que la satisfacin són isomorfs de manera única. Per això parlem de \enquote{l'} objecte terminal, \enquote{el} producte, etc., encara que n'hi pugui haver molts: tots són canònicament el mateix. Aquesta unicitat és el que dona sentit a caracteritzar objectes per la seva propietat universal en lloc de per una construcció concreta.
\end{observacio}
...
Observem primer com es llegeixen les nocions anteriors. Que $1$ sigui terminal vol dir que $\Hom(X,1)$ té exactament un element per a cada $X$; és a dir, que el functor $\Hom(-,1)$ és isomorf al functor constant amb valor un conjunt d'un sol element. Dualment, $0$ és inicial quan $\Hom(0,-)$ és el functor constant amb valor un punt.
...
\section{El plançó de Yoneda}

Fixem una categoria \textbf{petita} $\cat{C}$ (perquè la categoria de prefeixos i els seus homconjunts quedin ben definits sense recórrer a universos; per a $\cat{C}$ només localment petita, tot el que segueix val amb la cautela habitual sobre la mida de $\cat{Set}^{\cat{C}\op}$). A cada objecte $A$ li hem associat el functor contravariant representable $\Hom(-,A)\colon\cat{C}\op\to\cat{Set}$. Aquesta assignació és, ella mateixa, functorial: dona un functor de $\cat{C}$ cap a la categoria de functors $\cat{Set}^{\cat{C}\op}$, anomenada categoria de \emph{prefeixos} sobre $\cat{C}$.

\begin{definicio}[Plançó de Yoneda]\index{Yoneda!plançó}\index{prefeix@prefeix}
\label{def:planco-yoneda}
El \textbf{plançó de Yoneda} (en anglès, \emph{Yoneda embedding}) és el functor
\[
...
\begin{observacio}
Cal comprovar que $\yon(f)$ és realment natural i que $\yon$ preserva composició i identitats, però tot es redueix, un cop més, a l'associativitat i a les lleis de la identitat de $\cat{C}$. El contingut del plançó de Yoneda és que \emph{tota} categoria localment petita s'incrusta en una categoria de prefeixos, que és una categoria molt rica (té tots els límits i colímits, per exemple). El teorema següent en justifica el nom: $\yon$ és una incrustació plena i fidel.
\end{observacio}
...
\label{teo:yoneda}
Sigui $\cat{C}$ una categoria localment petita, $A$ un objecte de $\cat{C}$ i $P\colon\cat{C}\op\to\cat{Set}$ un prefeix. Aleshores hi ha una bijecció
\[
\Nat\big(\Hom(-,A),\,P\big)\;\cong\;P(A),
\]
natural en $A$ i en $P$. Explícitament, la bijecció envia una transformació natural $\eta$ a l'element $\eta_A(\id_A)\in P(A)$.
\end{teorema}
...
\]
Les dues assignacions són, doncs, inverses, i la naturalitat en $A$ i en $P$ es comprova de manera anàloga.
\end{prova}

\section{Conseqüències}

La primera conseqüència és que el plançó de Yoneda mereix el seu nom.

\begin{corollari}[El plançó de Yoneda és ple i fidel]\index{Yoneda!plançó ple i fidel}
Per a tota parella d'objectes $A,B$ de $\cat{C}$, el plançó $\yon$ dona una bijecció
\[
...
\begin{observacio}
Hi ha una versió covariant de tot plegat, obtinguda treballant amb $\cat{C}\op$ en lloc de $\cat{C}$: dona un plançó $\cat{C}\op\to\cat{Set}^{\cat{C}}$, també ple i fidel, i el lema $\Nat(\Hom(A,-),Q)\cong Q(A)$ per a functors covariants $Q\colon\cat{C}\to\cat{Set}$. Totes dues versions s'usen contínuament; la contravariant, amb prefeixos, és la que enllaça amb la teoria de feixos i la lògica categòrica que estudiarem més endavant.
\end{observacio}
...
\begin{resumcap}
\apartat{Cinc idees} (1)~Una propietat universal és la representabilitat d'un functor cap a $\cat{Set}$. (2)~L'objecte representant és únic llevat d'isomorfisme; la representació sencera, llevat d'un únic isomorfisme compatible. (3)~El plançó de Yoneda encasta $\cat{C}$ dins dels prefeixos de manera plena i fidel. (4)~El lema de Yoneda identifica $\Nat(\Hom(-,A),P)$ amb $P(A)$. (5)~Objectes inicials, terminals, productes i coproductes són casos de propietat universal.
